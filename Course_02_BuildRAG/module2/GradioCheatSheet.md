# Cheat Sheet: Building Apps with RAG (Gradio Framework Reference)

---

## Interface Class Core Arguments
Gradio's `Interface` class wraps any arbitrary Python function. The interface class relies on the following three core arguments:

| Argument | Definition |
| :--- | :--- |
| **fn** | The function to wrap. Each function parameter represents an input component, and the function's output should be either a single value or a tuple. Each element within the tuple corresponds directly to a specific output component. |
...
| **outputs** | The Gradio component(s) to use for the outputs of the function. |
...
Use the `launch()` method to launch the application. This method starts a simple web server that serves the interface. 

* **Public Sharing:** If you set `share=True` in `launch()`, you can share the application with the rest of the world. The system will set up a public URL allowing anyone worldwide to access your application. 
* **Execution Environment:** Users can interact with your app through this public URL, while all heavy computations and data operations run securely on your local machine.
